...
$jsReady = "
    $('#nameCheckForm').submit(function() {
        ...
        var hotword = $('#eeSearchHotword').val();
        var city = $('#eeSearchCity').val();
        var industry = $('#eeSearchIndustry').val();
        var han = /^[\u4e00-\u9fa5]+$/;
        if (hotword == '' || hotword == undefined) {
            alert('请填写要推荐公司名称的关键词');
            return false;
        } else {
            if (!han.test(hotword)) {
                alert('公司名称的关键词请填写简体汉字');
                $('#eeSearchhotword').focus();
                return false;
            }
        }
                ...
                <div class="named-box">
                    ...
                    <?php $form = ActiveForm::begin(['enableClientScript' => false, 'options'=>['class'=>'form-inline', 'data-toggle'=>'validator', 'id'=>'nameCheckForm']]); ?>
                        <div class="form-group input-select">
                        	<?= Html::activeTextInput($model, 'hotword', ['class'=>'form-control', 'id'=>'eeSearchHotword', 'placeholder'=>'请输入关键词，如：百度', 'required'=>true]); ?>
                            <a class="select-btn">
                                <i class="icon-down"></i>
                            </a>
                        </div>
...
//                                 $tmpClass = 'eeLogin';
                        	...
                        	<button type="submit" class="btn btn-primary <?= $tmpClass; ?>">进行核名</button>
                    <?php ActiveForm::end(); ?>
                </div>
                <h5>如：上海百度科技有限公司，关键词是“百度”，城市是“上海”，行业是“科技”</h5>
            </div>
        </div>
    </div>
    <div class="warp">
